...
bool can_user_join_chan(Channel *chan, Client *my_client, std::string password)
{
	if (chan->getHasPassword() == true)
	{
	...
	{
		if (chan->getMemberOfThisChan().size() >= chan->getLimitUserMode())
			return false;
	}
	if (chan->getCanalInviteOnlyMode() == true)
	{
		if (chan->is_member_invited(my_client->getNickname()) ==  false)
			return false;
	}
	return (true);
}
...
void		Command::join()
{
	...
	for (std::map<std::string, std::string>::iterator ita = chanKey.begin(); ita != chanKey.end(); ita++)
	{
		...
		for (it = my_client->getMyServer()->GetChannelList().begin(); it !=  my_client->getMyServer()->GetChannelList().end(); it++)
		{
			...
			{
				if (can_user_join_chan(it->first, my_client, password) ==  true)
				{
