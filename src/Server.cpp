...
void Server::redaction_answer_request(Command *my_command, std::string concerned_client_nick, Client *expediteur)
{
	...
	else if (!my_command->getErrorcode().empty() )
	{
		...
		buffer_to_send += " ";
		if (my_command->getErrorcode() != "441")
		{
			buffer_to_send += expediteur->getNickname();
			buffer_to_send += " ";
		}
...
{
	// std
	if (my_command->getStatus() == SINGLE_SEND)
	{
		for (size_t i = 0; i < my_command->getConcernedClients().size(); i++)
		{
			...
			if (desti == NULL)
				continue ;
			else if (my_command->getIs_ready() == true && my_command->getStatus() == SINGLE_SEND)
	...
	else if (my_command->getStatus() == NOT_ALL_SEND)
	{
...
bool Server::loop_running_server(void)
{
	...
	while (1)
	{
		...
		for (int i = 0; i < max_events; i++)
        {
			...
			else if (received_events[i].events && received_events[i].events == EPOLLERR)
			{
				std::cerr << "EPOLLERR CATCH" << std::endl;
			}
