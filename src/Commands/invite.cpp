...
void Command::invite()
{
	...
	if (my_client->getMyServer()->is_channel_on_server(channel) == true)
	{
		if (my_client->getMyServer()->is_member_of_this_chan(channel, my_client->getNickname()) ==  true)
		{
			...
			{
				//401 : target n'est pas dans le serveur
				fill_no_such_nick();
				return ;
			}
			...
			else if(my_client->getMyServer()->send_back_channel(channel)->getCanalInviteOnlyMode() == true
				&& my_client->getMyServer()->send_back_channel(channel)->is_member_operator(my_client->getNickname()) == false)
			{
				//482
				int socket = my_client->GetClientSocketFD();
				std::string full_buffer_client = Concerned_Buffers[socket];
			...
			else if (my_client->getMyServer()->is_member_of_this_chan(channel, target) == false)
			{
				//on invite l'user
				int socket = my_client->GetClientSocketFD();
				std::string full_buffer_client = Concerned_Buffers[socket];
