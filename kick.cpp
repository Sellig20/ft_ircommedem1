...
void Command::kick()
{
	...
	if (my_client->getMyServer()->is_channel_on_server(channel_from) == true)
	{
		if (my_client->getMyServer()->is_member_of_this_chan(channel_from, target) == true)
		{
			...
			full_buffer_client.append( ":" + target + "!" + target + "@localhost PART " + channel_from + " " + channel_from + "\r\n");
			if (!reason.empty())
		...
		else
		{
			...
			my_client->setRequestCode("441");
			error_code = "441";
			response_buffer = channel_from + " :KICK user [" + target + "] not on channel";
			is_ready = true;
